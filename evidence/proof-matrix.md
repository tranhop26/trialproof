...
The two sections are deliberately separated: the current 1.1.0 deployment and
happy-path rows prove deployed-source correspondence and live runtime behavior,
while local and CI regression rows cover the full admin-requested edge-case
logic. The historical 1.0.1 transactions do not prove 1.1.0 runtime behavior.

Known evidence gap: a live 1.1.0 important non-certifying branch has not yet
been executed. It must not be added to this matrix until its own transactions
are finalized successfully and contract state has been read back.
